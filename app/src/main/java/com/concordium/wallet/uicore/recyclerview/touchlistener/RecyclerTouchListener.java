...
//        mSwipeListener = listener;
    ...
    public RecyclerTouchListener setLongClickable(OnRowLongClickListener listener) {
    ...
    public RecyclerTouchListener setSwipeable(int foregroundID, int backgroundID, int backgroundWidth, OnSwipeOptionsClickListener listener) {
        this.swipeable = true;
        if (fgViewID != 0 && foregroundID != fgViewID)
            throw new IllegalArgumentException("foregroundID does not match previously set ID");
        fgViewID = foregroundID;
        bgViewID = backgroundID;
        bgViewWidth = backgroundWidth; //added this because getting width runtime always resulted on 0
