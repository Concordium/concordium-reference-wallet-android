...
interface EarnDelegate {
    fun gotoEarn(activity: Activity, account: Account, hasPendingDelegationTransactions: Boolean, hasPendingBakingTransactions: Boolean)
}

class EarnDelegateImpl : EarnDelegate {
    override fun gotoEarn(activity: Activity, account: Account, hasPendingDelegationTransactions: Boolean, hasPendingBakingTransactions: Boolean) {
        val intent: Intent
        if (account.accountDelegation != null || hasPendingDelegationTransactions) {
            ...
            intent.putExtra(EXTRA_DELEGATION_BAKER_DATA, BakerDelegationData(account, isTransactionInProgress = hasPendingDelegationTransactions, type = ProxyRepository.UPDATE_DELEGATION))

        } else if (account.accountBaker != null || hasPendingBakingTransactions) {
            intent = Intent(activity, BakerStatusActivity::class.java)
            intent.putExtra(EXTRA_DELEGATION_BAKER_DATA, BakerDelegationData(account, isTransactionInProgress = hasPendingBakingTransactions,  type = ProxyRepository.REGISTER_BAKER))
        } else {
