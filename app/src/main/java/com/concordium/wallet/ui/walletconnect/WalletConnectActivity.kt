...
class WalletConnectActivity : BaseActivity() {
    ...
    private fun initObservers() {
        ...
        viewModel.proofOfIdentitySuccess.observe(this) { proofs ->
            viewModel.respondSuccess(proofs)
            viewModel.stepPageBy.value = 1
        }
        viewModel.proofOfIdentityError.observe(this) { message ->
            viewModel.respondError(message)
        }
        viewModel.proofOfIdentityOkay.observe(this) {
            approveView()
        }
        ...
        viewModel.showAuthentication.observe(this) {
            showAuthentication(authenticateText(), object : AuthenticationCallback {
                ...
                override fun onCancelled() {
                    viewModel.authCanceled.postValue(true)
                }
            })
        }
    }
    ...
    private fun transactionView() {
        ...
        hideActionBarBack()
        setActionBarTitle(getString(R.string.wallet_connect_session_with))
        viewModel.walletConnectData.method = WalletConnectViewModel.SIGN_AND_SEND_TRANSACTION
        supportFragmentManager.beginTransaction().replace(
            R.id.fragment_container,
    ...
    private fun messageView() {
        ...
        hideActionBarBack()
        setActionBarTitle(getString(R.string.wallet_connect_session_with))
        viewModel.walletConnectData.method = WalletConnectViewModel.SIGN_MESSAGE
        supportFragmentManager.beginTransaction().replace(
            R.id.fragment_container,
