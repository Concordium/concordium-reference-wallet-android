...
class AccountItemView(context: Context, attrs: AttributeSet?): LinearLayout(context, attrs) {
    ...
    fun setAccount(accountWithIdentitiy: AccountWithIdentity) {
        ...
        balance_at_disposal_textview.text = CurrencyUtil.formatGTU(accountWithIdentitiy.account.totalUnshieldedBalance - accountWithIdentitiy.account.getAtDisposalSubtraction(), withGStroke = true)
