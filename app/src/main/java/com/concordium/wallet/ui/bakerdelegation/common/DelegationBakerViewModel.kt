...
class DelegationBakerViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private suspend fun createBakingTransaction(keys: AccountData, encryptionSecretKey: String?) {
        ...
        val expiry =
            (DateTimeUtil.nowPlusMinutes(10).time) / 1000 // Expiry should me now + 10 minutes (in seconds)
        ...
        val transactionFeeCommission =
            if (bakerDelegationData.type == REGISTER_BAKER || bakerDelegationData.type == CONFIGURE_BAKER)
                getTransactionRate(
                    bakerDelegationData.chainParameters?.transactionCommissionRange?.max ?: 0.0,
                    bakerDelegationData.chainParameters?.transactionCommissionRange?.min ?: 0.0
                ) else null
        val bakingRewardCommission =
            if (bakerDelegationData.type == REGISTER_BAKER || bakerDelegationData.type == CONFIGURE_BAKER)
                getTransactionRate(
                    bakerDelegationData.chainParameters?.bakingCommissionRange?.max ?: 0.0,
                    bakerDelegationData.chainParameters?.bakingCommissionRange?.min ?: 0.0
                ) else null
        val finalizationRewardCommission =
            if (bakerDelegationData.type == REGISTER_BAKER || bakerDelegationData.type == CONFIGURE_BAKER)
                getTransactionRate(
                    bakerDelegationData.chainParameters?.finalizationCommissionRange?.max ?: 0.0,
                    bakerDelegationData.chainParameters?.finalizationCommissionRange?.min ?: 0.0
                ) else null
    ...
    fun getTempFileWithPath(): Uri =
        Uri.parse("content://" + DataFileProvider.AUTHORITY + File.separator.toString() + FILE_NAME_BAKER_KEYS)

    fun getTransactionRate(max: Double, min: Double): Double = min + (max - min) * 0.1
}
